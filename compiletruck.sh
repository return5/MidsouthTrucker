...
printHelp() {
    printf "%s\n" "compile script for trucker"
    printf "%s\n" "compiles trucker and creates executable names 'trucker'"
...
useGCC() {
    printf "%s\n" "using GCC"
    gcc @src_files -Wall -Wextra -O2 -Iheaders -finline-functions -lm -Wswitch-enum -lncurses -std=gnu11 -otrucker
}

#compiles game using Clang
useClang() {
    printf "%s\n" "using Clang"
    clang @src_files -Iheaders -Wall -Wextra -O2 -lncurses -lm -finline-functions -Wswitch-enum -std=gnu11 -otrucker   
}
